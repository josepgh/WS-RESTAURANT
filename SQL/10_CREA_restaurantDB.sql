...
-- --DECLARE _id_taula INTEGER; NO ADMENT COMENTARIS EN EL TRIGGER???????????????????????????????????????????????????
DELIMITER //
...
/*
DELIMITER //
-- EN CREAR LA comanda, EL TRIGUER INSERTA EL id_personal PER SABER QUI HA SERVIT LA COMANDA
CCCCCCCCCCCREATE OR REPLACE TRIGGER insert_username_in_comanda
AFTER INSERT ON comandes
FOR EACH ROW
BEGIN
DECLARE _username_comanda VARCHAR;

SELECT username INTO _username_camanda FROM personal WHERE username = NEW.USER();

UPDATE comandes SET username_comanda =  _username_comanda WHERE ;  
...
/*
--TRIGGER DE burguerDB
DELIMITER //
-- EN CREAR LA RESERVA, EL TRIGUER INSERTA UNA COMANDA AMB L'ID DE LA RESERVA,
-- DATA I HORA DE LA RESERVA, L'ID DE LA TAULA I L'USUARI QUE FA LA RESERVA
-- AIXÍ QUEN LLIGATS LA RESERVA (cardinalitat 1) AMB LA COMANDA (1) AMB EL MATEIXOS IDS 
CREATE OR REPLACE TRIGGER insert_comanda_from_reserves
AFTER INSERT ON reserves
FOR EACH ROW
BEGIN

INSERT INTO comandes VALUES(DEFAULT, NEW.idReserva, NOW(), NEW.idTaula, USER());  
