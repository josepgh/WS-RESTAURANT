...
<body>
    ...
    <table>
		...
		<form action="prova_set_grants.php" method="post">
...
<!--     			<th>Grup grants</th>    			 -->
    ...
    <?php
        ...
        if (($_REQUEST['uname'] == "")){ //si l'username a cercar esta buit -> llista tots el personal
             ...
             $query = "SELECT p.id_personal, p.nom, p.rol, p.username, p.password, p.host, p.es_actiu, g.grants from personal p, grants_rol g WHERE p.rol = g.id_rol ORDER BY p.username";
...
//                             echo "<td>" . setGrants($row['rol'], $row['username'], $row['host'], $row['es_actiu']) . "</td>";
...
?>
	...
	<br><br>
	<form action="./prova_set_grants.php">
